...
async function main() {
    ...
    // Jenis Pembayaran
    const transfer = await prisma.jenis_pembayarans.create({
        data: { metode_pembayaran: 'Transfer Bank' },
    }).catch(() => prisma.jenis_pembayarans.findFirst({ where: { metode_pembayaran: 'Transfer Bank' } }));
    ...
    await prisma.detail_jenis_pembayarans.createMany({
        data: [
            { id_jenis_pembayaran: transfer.id, no_rek: '1234567890', tempat_bayar: 'BCA - A/N Catering P1', logo: 'bca.png' },
            { id_jenis_pembayaran: transfer.id, no_rek: '9876543210', tempat_bayar: 'BRI - A/N Catering P1', logo: 'bri.png' },
        ],
        skipDuplicates: true,
    });
